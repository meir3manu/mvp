...
function init() {
    answerCounts = { a: 0, b: 0, c: 0, d: 0, e: 0, f: 0, g: 0 }; // Adicione 'g' aqui
...
function showResult() {
    ...
    // Exibindo para depurar
    console.log('totalAnswers:', totalAnswers);
    console.log('sortedAnswers:', sortedAnswers);

    const resultMap = {
        a: '<b>Cibersegurança</b>: Protege sistemas, redes e dados contra ameaças digitais e ataques.',
        b: '<b>Blockchain</b>: Tecnologia que garante segurança e transparência em transações digitais.',
        c: '<b>Gestão de TI</b>: Administração de recursos de tecnologia da informação.',
        d: '<b>Dados</b>: Análise e gestão de grandes volumes de informações.',
        e: '<b>Programação</b>: Criação de software e aplicativos.',
        f: '<b>Design/Jogos Digitais</b>: Desenvolvimento de interfaces e experiências visuais.',
        g: '<b>Inteligência Artificial</b>: Criação de sistemas que simulam a inteligência humana.'
    };
    ...
    const resultHtml = sortedAnswers.slice(0, 2).map(([letter, count]) => {
        ...
        console.log(`Resultado para ${letter}: ${percentage}%`);  // Para verificar a porcentagem
        return `<li>${resultMap[letter] || 'Resultado não definido.'} <br><br> Você tem ${percentage}% de compatibilidade para esta área.<br></li>`;
    }).join('<br>');

    document.querySelector('#display-score').innerHTML = `
        <h3> O seu perfil ideal para as áreas de:</h3>
        <ul>${resultHtml}</ul>
    `;
